...
3. Subagent uses collector/push paths per `agents/agent-reflect-daily.md` — bash-safe `"${CLAUDE_PLUGIN_ROOT}${CLAUDE_PLUGIN_ROOT:+/}skills/.../scripts/..."`, plugin-root `scripts/*.sh` wrappers, or marketplace `scripts/*.py` launchers when `CLAUDE_PLUGIN_ROOT` is missing.
